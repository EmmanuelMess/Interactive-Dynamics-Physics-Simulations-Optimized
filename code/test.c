...
void testSymdiff() {
	{
		...
		const float value = SymbolNodeEvaluate(expression, 0, 100);
		...
		const float valueD = SymbolNodeEvaluate(derivate, 0, 100);
		...
		const float valueD2 = SymbolNodeEvaluate(derivate2, 0, 100);
...

	{
		...
		const float value = SymbolNodeEvaluate(d->values[0], 0, 100);

		printf("R %f\n", value);

		SymbolMatrix* derivate = SymbolMatrixDifferentiateSymbolNode(d, &arraySymbolMatrix, a->values[0]);

		const float valueD = SymbolNodeEvaluate(derivate->values[0], a->values[0], 100);
		...
		const float valueD2 = SymbolNodeEvaluate(derivate2->values[0], a->values[0], 100);
